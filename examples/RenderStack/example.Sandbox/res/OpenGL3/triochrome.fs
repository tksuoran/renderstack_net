...
void main(void)
{
    ...
    float r2 = 1.0 - r1;
    float deghost = material.deghost * 0.275;

    accum = sampleR * vec4(r1, r2, 0.0, 1.0);
    image.r = pow(accum.r + accum.g + accum.b, 1.15);
    image.a = accum.a;

    accum = sampleL * vec4(l2, l1, l2, 1.0);
    image.g = pow(accum.r + accum.g + accum.b, 1.05);
    image.a = image.a + accum.a;

    accum = sampleR * vec4(0.0, r2, r1, 1.0);
    image.b = pow(accum.r + accum.g + accum.b, 1.15);
    image.a = (image.a + accum.a) / 3.0;

    accum = image;
    image.r = (accum.r + (accum.r * (deghost *  0.50)) + (accum.g * (deghost * -0.25)) + (accum.b * (deghost * -0.25)));
    image.g = (accum.g + (accum.r * (deghost * -0.50)) + (accum.g * (deghost *  0.25)) + (accum.b * (deghost * -0.50)));
