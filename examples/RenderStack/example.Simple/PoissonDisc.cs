...
namespace example.Simple
{
    ...
    public static class UniformPoissonDiskSampler
    {
        ...
        public static List<Vector2> SampleCircle(
            Vector2 center, 
            float   radius, 
        ...
        public static List<Vector2> SampleRectangle(
            Vector2 topLeft, 
            Vector2 lowerRight, 
            float   minimumDistance
        )
        ...
        static void AddFirstPoint(ref Settings settings, ref State state)
        {
            ...
            while(!added)
            {
                ...
                if(
                    ...
                    (settings.Center.DistanceSquared(p) > settings.RejectionSqDistance)
                )
                {
                    continue;
                }
        ...
        )
        {
            ...
            if(
                ...
                (
                    settings.RejectionSqDistance == null || 
                    settings.Center.DistanceSquared(q) <= settings.RejectionSqDistance
                )
            )
            {
                ...
                )
                {
                    ...
                    {
                        if(
                            (state.Grid[i, j].HasValue) && 
                            (state.Grid[i, j].Value.Distance(q) < settings.MinimumDistance)
                        )
                        {
                            tooClose = true;
                        }
